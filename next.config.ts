...
const nextConfig: NextConfig = {
  ...
  images: {
    ...
    // 許可する品質を列挙する。既定は「何でも通す」。
    //
    // /_next/image は誰でも叩ける。remotePatterns で対象は自分のバケットに
    // 絞ってあるが、q を指定し放題だと
    //   127点 × 幅10種 × 品質100種 ≒ 12万通り
    // のユニーク変換を外から作れてしまう。
    // Cloudflare Images の無料枠は月5,000ユニーク変換で、超えると新規変換が
    // エラーになる（課金ではなく）。つまり画像が出なくなる。焼き切るのは容易。
    //
    // 実際に使っているのは3つだけ。これ以外は 400 で弾く。
    //   75  カード・詳細（next/image の既定。WorkImage は未指定）
    //   80  OGP（works/[id]/page.tsx の generateMetadata）
    //   85  ライトボックス（同ページの原寸表示）
    //
    // 増やす時はここに足すこと。載っていない値を渡すと画像が出ない。
    qualities: [75, 80, 85],
  },
};
